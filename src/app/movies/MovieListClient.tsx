"use client";

import { useEffect, useState } from "react";
import axios from "axios";
import Link from "next/link";
import { motion, AnimatePresence } from "framer-motion";
import Navbar from "../navbar";

export const runtime = 'edge';

// Set the base URL for axios
const api = axios.create({
  baseURL: process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000",
});

interface Movie {
  id: number;
  title: string;
  description: string;
  poster_url: string;
  release_year: number;
  translator: string;
  language: string;
}

interface Genre {
  id: number;
  name: string;
  movies: Movie[];
}

// Skeleton Card Component
const SkeletonCard = () => (
  <div className="flex-shrink-0 w-48 md:w-56 bg-gray-800 rounded-lg overflow-hidden">
    <div className="relative h-64 md:h-72 bg-gray-700 animate-pulse"></div>
    <div className="p-4 space-y-2">
      <div className="h-4 bg-gray-700 rounded w-3/4 animate-pulse"></div>
      <div className="h-3 bg-gray-600 rounded w-1/2 animate-pulse"></div>
      <div className="h-3 bg-gray-600 rounded w-1/3 animate-pulse"></div>
    </div>
  </div>
);

// Animation Variants
const cardVariants = {
  hidden: { opacity: 0, y: 20 },
  visible: (i: number) => ({
    opacity: 1,
    y: 0,
    transition: {
      delay: i * 0.1, // Stagger effect
      duration: 0.3,
    },
  }),
};

export default function MoviesPage() {
  const [genres, setGenres] = useState<Genre[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  // State to track visible cards per genre
  const [visibleCardsByGenre, setVisibleCardsByGenre] = useState<{ [key: number]: number }>({});

  // Number of skeleton cards to show per genre initially
  const SKELETON_COUNT = 4;

  // Format genre names to match desired style
  const formatGenreName = (name: string) => {
    const genreMap: { [key: string]: string } = {
      Action: "Action Films",
      Drama: "Drama Films",
      "Sci-Fi": "Sci-Fi Films",
      Adventure: "Adventure Films",
      Thriller: "Thriller Films",
      Comedy: "Comedy Films",
      Fantasy: "Fantasy Films",
      Horror: "Horror Films",
      Romance: "Romantic Films",
      Mystery: "Mystery Films",
    };
    return genreMap[name] || `${name} Films`;
  };

  useEffect(() => {
    const fetchMoviesByGenre = async () => {
      try {
        setLoading(true);
        const genresResponse = await api.get("/genres");
        const allGenres = genresResponse.data;

        const genresWithMovies = await Promise.all(
          allGenres.map(async (genre: { id: number; name: string }) => {
            try {
              const moviesResponse = await api.get(`/movies-by-genre/${genre.id}`);
              const movies = Array.isArray(moviesResponse.data.movies)
                ? moviesResponse.data.movies
                : [];
              return {
                id: genre.id,
                name: formatGenreName(genre.name),
                movies,
              };
            } catch (err) {
              console.error(`Error fetching movies for genre ${genre.id}:`, err);
              return { id: genre.id, name: formatGenreName(genre.name), movies: [] };
            }
          })
        );

        const filteredGenres = genresWithMovies.filter((genre) => genre.movies.length > 0);
        setGenres(filteredGenres);

        // Initialize visible cards for each genre
        const initialVisible = filteredGenres.reduce((acc, genre) => {
          acc[genre.id] = 0;
          return acc;
        }, {} as { [key: number]: number });
        setVisibleCardsByGenre(initialVisible);

        // Simulate progressive loading for each genre
        const interval = setInterval(() => {
          setVisibleCardsByGenre((prev) => {
            const updated = { ...prev };
            let allLoaded = true;

            filteredGenres.forEach((genre) => {
              if (updated[genre.id] < genre.movies.length) {
                updated[genre.id] += 1;
                allLoaded = false;
              }
            });

            if (allLoaded) {
              clearInterval(interval);
              setLoading(false);
            }

            return updated;
          });
        }, 200); // Add one card every 200ms
      } catch (err) {
        console.error("Error fetching genres or movies:", err);
        setError("Failed to load movies. Please try again later.");
        setLoading(false);
      }
    };

    fetchMoviesByGenre();
  }, []);

  if (error) {
    return (
      <div className="flex justify-center items-center h-screen bg-gradient-to-b from-gray-900 to-gray-800">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
          className="text-2xl font-semibold text-red-400"
        >
          {error}
        </motion.div>
      </div>
    );
  }

  return (
    <>
      <Navbar />
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-white pt-16">
        <div className="max-w-7xl mx-auto px-4 py-12">
          <motion.h1
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="text-4xl md:text-5xl font-extrabold text-center mb-12 bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-600"
          >
            Movies by Genre
          </motion.h1>
          {genres.length === 0 && !loading ? (
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.5 }}
              className="text-center text-gray-400 text-lg"
            >
              No movies found.
            </motion.p>
          ) : (
            genres.map((genre) => (
              <motion.div
                key={genre.id}
                className="mb-12"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: 0.2 * genre.id }}
              >
                <h2 className="text-2xl md:text-3xl font-bold mb-6 text-gray-100 border-l-4 border-purple-500 pl-4">
                  {genre.name}
                </h2>
                <div className="flex overflow-x-auto gap-6 pb-4 scrollbar-thin scrollbar-thumb-purple-500 scrollbar-track-gray-700">
                  <AnimatePresence>
                    {loading && visibleCardsByGenre[genre.id] === 0
                      ? // Show skeleton cards during loading
                        Array.from({ length: SKELETON_COUNT }).map((_, index) => (
                          <motion.div
                            key={`skeleton-${genre.id}-${index}`}
                            variants={cardVariants}
                            initial="hidden"
                            animate="visible"
                            custom={index}
                          >
                            <SkeletonCard />
                          </motion.div>
                        ))
                      : // Show actual cards up to visibleCardsByGenre
                        genre.movies.slice(0, visibleCardsByGenre[genre.id] || 0).map((movie, index) => (
                          <motion.div
                            key={`${movie.id}`}
                            variants={cardVariants}
                            initial="hidden"
                            animate="visible"
                            custom={index}
                            className="flex-shrink-0 w-48 md:w-56"
                          >
                            <Link href={`/movies/${encodeURIComponent(movie.title)}`} className="block">
                              <motion.div
                                className="bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2"
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                              >
                                <div className="relative h-64 md:h-72">
                                  <img
                                    src={movie.poster_url || "/placeholder-movie.png"}
                                    alt={movie.title}
                                    className="w-full h-full object-cover rounded-t-lg"
                                    onError={(e) => {
                                      (e.target as HTMLImageElement).src = "/placeholder-movie.png";
                                    }}
                                  />
                                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                                    <p className="text-sm text-white font-medium truncate">{movie.description}</p>
                                  </div>
                                </div>
                                <div className="p-4">
                                  <h3 className="font-semibold text-white truncate">{movie.title}</h3>
                                  <p className="text-sm text-gray-400">{movie.release_year}</p>
                                  <p className="text-xs text-gray-500 truncate">{movie.language}</p>
                                </div>
                              </motion.div>
                            </Link>
                          </motion.div>
                        ))}
                  </AnimatePresence>
                </div>
              </motion.div>
            ))
          )}
        </div>
      </div>
    </>
  );
}
